...
function resolve(source, file, options) {
    ...
    var foundNodePath;
    try {
        foundNodePath = tsResolve(mappedPath !== null && mappedPath !== void 0 ? mappedPath : source, __assign(__assign({}, options), { extensions: (_a = options.extensions) !== null && _a !== void 0 ? _a : defaultExtensions, basedir: path__default['default'].dirname(path__default['default'].resolve(file)), packageFilter: (_b = options.packageFilter) !== null && _b !== void 0 ? _b : packageFilterDefault }));
    }
    catch (_c) {
        foundNodePath = null;
    }
    ...
    if ((/\.jsx?$/.test(foundNodePath) ||
        ...
        !/^@types[/\\]/.test(source) &&
        !path__default['default'].isAbsolute(source) &&
        !source.startsWith('.')) {
        var definitelyTyped = resolve('@types' + path__default['default'].sep + mangleScopedPackage(source), file, options);
...
function initMappers(options) {
    ...
    mappers = configPaths
        ...
        .reduce(function (paths, path) { return __spreadArray(__spreadArray([], paths), (isGlob__default['default'](path) ? glob.sync(path) : [path])); }, [])
...
function mangleScopedPackage(moduleName) {
    if (moduleName.startsWith('@')) {
        var replaceSlash = moduleName.replace(path__default['default'].sep, '__');
